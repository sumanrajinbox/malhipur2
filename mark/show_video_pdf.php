    ...
    v.status = 1";
            ...
            $video_data=$conn->select_all_row($sql_video_pdf);
                ...
                foreach($video_data as $video) {
                    $pdf_data=$conn->select_all_row("select pdf_name,pdf_url from pdf where video_id = '".$video['video_id']."' and status = 1");
            ...
            array_push($all_data, $result);
                ...
                if($all_data) {
                    ...
                    $response_video_pdf["video_pdf_data"]=$all_data; 
                    header('HTTP/1.1 200 success', true, 200);
                    header('Content-Type: application/json');
                    echo json_encode($response_video_pdf);
                }
                else {
                    $response["error"]=TRUE;
