...
if($_SERVER['REQUEST_METHOD']==='POST'&& isset($data['email'])) {
	$check_email = $conn->select_one_row('select * from login where email = "'.$data['email'].'"');
	...
	if($check_email) {
		...
		$to=$conn->select_one_row('select * from login as l  where l.id = "'.$user_id.'"');		
		...
		$user_exist=$conn->select_one_row('SELECT * FROM `admin_forgot` where user_id = "'.$user_id.'" ');
		...
		if($user_exist) {
			...
			$insert_otp=$conn->only_query('INSERT INTO `admin_forgot`(`id`, `user_id`, `otp`, `active`, `dated`) 
			VALUES
			 ("","'.$user_id.'","'.$generate_otp.'",1,"'.date("Y-m-d H:i:s").'")');
			//echo 'otp inserted';		
		
		}

		$to = $to['email'];
$subject = ' Admin password OTP by challenge education ';
...
if($send_result){
	$response["error"]=FALSE;
		$response["message"]="Admin OTP has been sent in your Registered Email id ".$mask_email;
