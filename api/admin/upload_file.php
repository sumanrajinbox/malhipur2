...
for($key=0; $key < $total_files; $key++) {
    ...
    if(isset($video_file['name'][$key]) && $video_file['size'][$key] > 0) {
        ...
        $thumb_rename=$new_name.date('YmdHis')."_".$key.".".$file_ext_thumb;
        ...
        if(empty($errors)==true) {
            ...
            echo $last_id=$conn->only_query("SELECT * LAST_INSERT_ID()");
            // =================================================================
        ...
        for($key_pdf=0; $key_pdf < $total_pdf_files; $key_pdf++) { 
            ...
            $pdf_rename=$new_name.date('YmdHis')."_".$key_pdf.".".$file_ext_pdf;
