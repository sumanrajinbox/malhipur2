...
if (isset($post['email']) && isset($post['fcm']) && isset($post['deviceid']) && isset($post['route'])) {
	$email = $post['email'];
	$fcm = $post['fcm'];
	...
	// 			echo "social login";
	// 			//// Social Login
	// 			$row = $Dbcon->select_one_row("select * from users where email='" . $email . "' and active='1'");
	// 			if ($row) {
	// 			///success
	// 			$auth = $apikey->apigen($row['id'], $fcm, $deviceid, $email);
	// 			//echo "SELECT users.id,users.email,users.created,profile.name,profile.dob,profile.height,profile.weight,profile.profile_pic,profile.modified,auth.auth_key FROM users LEFT JOIN profile on users.id=profile.user_id LEFT JOIN auth on users.id=auth.user_id WHERE users.id='".$row['id']."' and auth.device_id='".$deviceid."' and users.active='1'";
	// 		 //				exit();
	...
	// 		}
	// 		if ($user['dob']) {
	// 			$dob = $convert->normalDate($user['dob']);
	// 		}
	// 		$response["error"] = FALSE;
	// 		$response["message"] = "Successful login";
	// 		$response["auth_key"] = $user['auth_key'];
	...
	$response["error"] = TRUE;
	$response["message"] = "Required parameters (email or password, FCM, Device ID, Route) is missing!";
