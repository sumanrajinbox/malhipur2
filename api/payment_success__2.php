...
if (isset($auth) && $auth != "" && $data['result']['txnid'] != "") {
    ...
    $cd  =  $conn->select_one_row('select month_id ,class_id from course_details where user_id = "' . $auth['user_id'] . '"  and active = 1 ');
    $fee_details = $conn->select_one_row('select * from fee where monthid = "' . str_replace(' ', '', $month_id[0]) . '" and classid = "' . $cd['class_id'] . '"');
    $get_redeem_id = $conn->select_one_row('select * from profile where user_id = "' . $auth['user_id'] . '"');
    $referral_code = $conn->select_one_row('select * from referral_account where r_code = "' . $get_redeem_id['referral_code'] . '"');
    $referral_code_data = $conn->select_one_row('select * from referral_account where redeem_id = "' . $referral_code['r_code'] . '" and r_code = "'. $get_redeem_id['referral_code'].'"');
    if($referral_amount){
    ...
    if ($cd['month_id'] != "") {
        ...
        $fee = $last_price + $fee;
        $update_referral_prrson_price = $conn->only_query('update referral_account set referral_price = "' . $fee . '" where r_code = "' . $referral_code['r_code'] . '"');
        $month_id = str_replace(' ', '', implode(",", array_filter($month_id)));
        $update_course_details = 'update course_details set paid = 1 ,month_id = "' . $month_id . '"where user_id = "' . $auth['user_id'] . '"';
        $update_course_details_result = $conn->only_query($update_course_details);
    }
